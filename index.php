...
<head>
    ...
    <script>
        function getGeoParams(callback) { // приймає як аргумент функцію зворотного виклику callback
            if (navigator.geolocation) { 
                navigator.geolocation.getCurrentPosition(
                (position) => {
                    callback({ // якщо успішно
        ...
        function fetchResults() {
            ...
            getGeoParams(function (geo) {
                var xhr = new XMLHttpRequest(); 
                var url = "get1.php?publisher=" + encodeURIComponent(publisher) + // формування URL до get1.php
        ...
        function fetchBooks() {
            ...
            if (startYear && endYear) {
                getGeoParams(function (geo) { // визначаємо кординати 
                    var xhr = new XMLHttpRequest(); 
                    var url = "get2.php?start_year=" + startYear + "&end_year=" + endYear +
                              "&lat=" + encodeURIComponent(geo.lat) +
                              "&lon=" + encodeURIComponent(geo.lon);
                    xhr.open('GET', url, true);
...
<body>
    ...
    getGeoParams(function (geo) {
        ...
        script.src = "get3.php?author_id=" + authorId +
                     "&lat=" + encodeURIComponent(geo.lat) +
                     "&lon=" + encodeURIComponent(geo.lon) +
